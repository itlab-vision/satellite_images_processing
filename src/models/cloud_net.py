...
class CloudNet():
    ...
    def contr_arm(self, input_tensor, filters, kernel_size):
        ...
        filters_b = filters // 2
        # creates a kernl size of (1,1) out of (3,3)
    ...
    def imprv_contr_arm(self, input_tensor, filters, kernel_size):
        """It adds a feedforward signal to the output of two following conv layers in contracting path
        """
    ...
    def bridge(self, input_tensor, filters, kernel_size):
    ...
    def conv_block_exp_path(self, input_tensor, filters, kernel_size):
    ...
    def conv_block_exp_path3(self, input_tensor, filters, kernel_size):
        """It Is only the convolution part inside each expanding path's block
        """
        x = Conv2D(filters, kernel_size, padding='same')(input_tensor)
        x = self.bn_relu(x)
    ...
    def model_arch(self, input_rows=192, input_cols=192, num_of_channels=4, num_of_classes=1):
        ...
        convT7 = Conv2DTranspose(
            512, (2, 2), strides=(2, 2), padding='same')(conv6)
        prevup7 = self.improve_ff_block4(input_tensor1=conv4, input_tensor2=conv3,
                                         input_tensor3=conv2, input_tensor4=conv1, pure_ff=conv5)
        up7 = concatenate([convT7, prevup7], axis=3)
        conv7 = self.conv_block_exp_path3(
            input_tensor=up7, filters=512, kernel_size=(3, 3))
        conv7 = self.add_block_exp_path(conv7, conv5, convT7)

        convT8 = Conv2DTranspose(
            256, (2, 2), strides=(2, 2), padding='same')(conv7)
        prevup8 = self.improve_ff_block3(
            input_tensor1=conv3, input_tensor2=conv2, input_tensor3=conv1, pure_ff=conv4)
        up8 = concatenate([convT8, prevup8], axis=3)
        conv8 = self.conv_block_exp_path(
            input_tensor=up8, filters=256, kernel_size=(3, 3))
        conv8 = self.add_block_exp_path(
            input_tensor1=conv8, input_tensor2=conv4, input_tensor3=convT8)

        convT9 = Conv2DTranspose(
            128, (2, 2), strides=(2, 2), padding='same')(conv8)
        prevup9 = self.improve_ff_block2(
            input_tensor1=conv2, input_tensor2=conv1, pure_ff=conv3)
        up9 = concatenate([convT9, prevup9], axis=3)
        conv9 = self.conv_block_exp_path(
            input_tensor=up9, filters=128, kernel_size=(3, 3))
        conv9 = self.add_block_exp_path(
            input_tensor1=conv9, input_tensor2=conv3, input_tensor3=convT9)

        convT10 = Conv2DTranspose(
            64, (2, 2), strides=(2, 2), padding='same')(conv9)
        prevup10 = self.improve_ff_block1(input_tensor1=conv1, pure_ff=conv2)
        up10 = concatenate([convT10, prevup10], axis=3)
        conv10 = self.conv_block_exp_path(
            input_tensor=up10, filters=64, kernel_size=(3, 3))
        conv10 = self.add_block_exp_path(
            input_tensor1=conv10, input_tensor2=conv2, input_tensor3=convT10)

        convT11 = Conv2DTranspose(
            32, (2, 2), strides=(2, 2), padding='same')(conv10)
        up11 = concatenate([convT11, conv1], axis=3)
        conv11 = self.conv_block_exp_path(
            input_tensor=up11, filters=32, kernel_size=(3, 3))
        conv11 = self.add_block_exp_path(
            input_tensor1=conv11, input_tensor2=conv1, input_tensor3=convT11)
